...
export default function ProductForm({ initial, onSave }: ProductFormProps) {
  ...
  const [form, setForm] = useState({
    ...
    slug: init?.slug as string || '',
    subtitle: init?.subtitle as string || '',
    descriptionHe: init?.descriptionHe as string || '',
    descriptionShort: init?.descriptionShort as string || '',
    benefitsList: ((init?.benefitsList as string[]) || []).join('\n'),
    ctaText: init?.ctaText as string || '',
    addToCartText: init?.addToCartText as string || '',
    videoUrl: init?.videoUrl as string || '',
    ogImage: init?.ogImage as string || '',
    scheduledAt: init?.scheduledAt ? new Date(init.scheduledAt as string).toISOString().slice(0, 16) : '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await onSave({
      ...
      benefitsList: form.benefitsList.split('\n').map(s => s.trim()).filter(Boolean),
      ctaText: form.ctaText,
      addToCartText: form.addToCartText,
